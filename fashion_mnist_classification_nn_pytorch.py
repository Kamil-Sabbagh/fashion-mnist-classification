...
class FashionMNISTDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        datasets.FashionMNIST('F_MNIST_data', download=True, train=True, transform=self.transform)
        datasets.FashionMNIST('F_MNIST_data', download=True, train=False, transform=self.transform)

    def setup(self, stage=None):
        train_ds = datasets.FashionMNIST('F_MNIST_data', train=True, transform=self.transform)
        test_ds = datasets.FashionMNIST('F_MNIST_data', train=False, transform=self.transform)
...
class FashionMNISTClassifier(pl.LightningModule):
    def __init__(self, optimizer_name="adam", learning_rate=0.003):
        ...
        self.model = nn.Sequential(OrderedDict([('fc1', nn.Linear(784, 128)),
                                                ('relu1', nn.ReLU()),
...
# Initialize DataModule and LightningModule
#data_module = FashionMNISTDataModule(batch_size=64)
#model = FashionMNISTClassifier()

# Initialize Trainer with desired configuration
#trainer = Trainer(max_epochs=25, devices=1 ,accelerator="gpu" if torch.cuda.is_available() else "cpu")
...
# fashion_mnist_data.setup(stage="test")


# test_dl = fashion_mnist_data.test_dataloader()
...
# trainer.test(model, datamodule=fashion_mnist_data)
...
# # Initialize Trainer with desired configuration
# trainer2 = Trainer(max_epochs=25, devices=1 ,accelerator="gpu" if torch.cuda.is_available() else "cpu")
...
# trainer2.test(model2, datamodule=fashion_mnist_data)